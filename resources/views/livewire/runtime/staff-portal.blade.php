<div>
    <div class="fade-in-up">
        <!-- Welcome Header -->
        <div class="mb-10">
            <p class="text-[14px] font-semibold text-green-600 mb-1">Good {{ now()->hour < 12 ? 'Morning' : (now()->hour < 17 ? 'Afternoon' : 'Evening') }},</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            @forelse($features as $featureVersion)
                ...
                <div class="group relative bg-white border border-black/[0.04] shadow-[0_2px_12px_rgb(0,0,0,0.02)] rounded-[20px] overflow-hidden hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:-translate-y-0.5 transition-all duration-300 {{ $isUnavailable ? 'opacity-50' : '' }}">
                    <!-- Top accent bar -->
                    <div class="h-1 bg-gradient-to-r {{ $featureVersion->availability === 'available' ? 'from-green-400 to-emerald-500' : ($featureVersion->availability === 'degraded' ? 'from-lime-400 to-green-500' : 'from-rose-400 to-red-500') }}"></div>

                    <div class="p-6">
                        <div class="flex items-start justify-between mb-4">
                            <div class="w-11 h-11 rounded-[12px] {{ $featureVersion->availability === 'available' ? 'bg-gradient-to-br from-green-50 to-emerald-50 border-green-100/60' : ($featureVersion->availability === 'degraded' ? 'bg-gradient-to-br from-lime-50 to-green-50 border-lime-100/60' : 'bg-gradient-to-br from-rose-50 to-red-50 border-rose-100/60') }} border flex items-center justify-center shadow-sm">
                                @if(str_contains(strtolower($featureVersion->feature->key), 'pledge') && str_contains(strtolower($featureVersion->feature->key), 'new'))
                                    <svg class="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M12 4v16m8-8H4" /></svg>
                                @elseif(str_contains(strtolower($featureVersion->feature->key), 'redemption'))
                                    <svg class="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 14l6-6m-5.5.5h.01m4.99 5h.01M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16l3.5-2 3.5 2 3.5-2 3.5 2z" /></svg>
                                @elseif(str_contains(strtolower($featureVersion->feature->key), 'renewal'))
                                    <svg class="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                                @elseif(str_contains(strtolower($featureVersion->feature->key), 'payment'))
                                    <svg class="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                                @else
                                    <svg class="w-5 h-5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                                @endif
                            </div>
                            <span class="px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full {{ $featureVersion->availability === 'available' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200/50' : ($featureVersion->availability === 'degraded' ? 'bg-green-50 text-green-700 border border-green-200/50' : 'bg-rose-50 text-rose-700 border border-rose-200/50') }}">
                        ...
                        @else
                            <a href="{{ route('portal.operations.launch', ['featureKey' => $featureVersion->feature->key]) }}" class="mt-2 flex items-center text-[13px] font-bold text-green-600 group-hover:text-green-700 transition-colors">
                                Launch
